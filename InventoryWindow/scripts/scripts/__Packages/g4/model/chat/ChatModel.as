class g4.model.chat.ChatModel extends g4.model.Model
{
   ...
   function $OnGame_Chat_SetWriting(channelID, writing, id, addParam, tabIDList, wisperType, time)
   {
      var _loc4_ = g4.model.chat.ChatUtil.convertChatMessage(channelID,writing,id,addParam,tabIDList,wisperType);
      if(_loc4_ == null || _loc4_.length == 0)
      {
         return undefined;
      }
      time = "<font color=\'" + g4.model.GameModel.chatModel.getChatChannelData(channelID).shapColor + "\'>[" + time + "]</font>";
      var _loc10_ = lib.util.ExtString.split(tabIDList,"\t");
      if(_loc10_.length <= 0)
      {
         _loc10_.push(tabIDList);
      }
      ...
      while(_loc5_ < _loc12_)
      {
         ...
         var _loc3_ = this.getChatTabData(_loc7_);
         var _loc6_ = false;
         var _loc8_ = lib.gamedata.CustomizedUIDataCollection.getInstance().valueOf(lib.gamedata.CustomizedUIDataKey.MESSAGE_TIMESTAMP_VISIBLE) == 1;
         if(_loc8_ && !_loc9_)
         {
            _loc9_ = true;
            _loc4_[0] = time + _loc4_[0];
         }
         _loc3_.chatTextArray = _loc3_.chatTextArray.concat(_loc4_);
