class g4.manager.MouseEventManager extends g4.core.EventDispatcher
{
   ...
   function addTargetEventListener(target, eventType, scope, method)
   {
   ...
   function removeTargetEventListener(target, eventType, scope, method)
   {
   ...
   function dispatchTargetEvent(targetPath, eventType)
   {
      ...
      while(_loc2_ < _loc3_.length)
      {
         ...
         _loc4_.apply(_loc5_,[_loc3_[_loc2_].target]);
         _loc2_ = _loc2_ + 1;
      }
   }
   ...
   {
      this.dispatchEvent(g4.manager.MouseEventManager.MOUSE_WHEEL,[delta,target]);
   ...
   function onMouseMove()
   {
      if(_global.gbUIMode)
      {
         ...
         this.dispatchEvent(g4.manager.MouseEventManager.MOUSE_MOVE,[_loc5_]);
   ...
   function onMouseDown(button, targetPath, mouseIdx, x, y, dblClick)
   {
      ...
      if(!this._mouseClicked)
      {
         ...
         {
            this.dispatchEvent(g4.manager.MouseEventManager.MOUSE_DOWN,[button,targetMc,targetPath]);
            _root.OnGameEvent("OnGame_MouseDown",button,targetMc,targetPath);
            if(button == Mouse.RIGHT)
            {
               this.dispatchTargetEvent(targetPath(targetMc),g4.manager.MouseEventManager.RIGHT_BUTTON_DOWN);
            }
   ...
   function onMouseUp(button, targetPath, mouseIdx, x, y)
   {
      ...
      this._mouseClicked = false;
      this.dispatchEvent(g4.manager.MouseEventManager.MOUSE_UP,[button,eval(targetPath)]);
      ...
      {
         this.dispatchTargetEvent(targetPath(target),g4.manager.MouseEventManager.RIGHT_BUTTON_CLICK);
      }
   ...
   function doubleClickCheck(target, button)
   {
      ...
      else if(this._clickCnt == 2)
      {
         ...
         {
            this.dispatchTargetEvent(targetPath(target),g4.manager.MouseEventManager.LEFT_BUTTON_DOUBLE_CLICK);
         }
         this._clickCnt = 0;
         clearInterval(this._doubleClickInterval[0]);
         this._doubleClickInterval.shift();
