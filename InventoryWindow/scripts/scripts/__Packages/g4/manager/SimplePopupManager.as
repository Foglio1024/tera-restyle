   ...
   function showSimplePopup(simplePopupType, simplePopupP)
   {
      ...
      if(this._simplePopupMap[_loc4_] == null)
      {
         ...
         _loc10_.onLoadInit = function(target)
         {
            ...
            target._x = (Stage.width - _loc3_.x * (lib.info.AlignInfo.scaleRatio / 0.8)) * 0.5;
            target._y = (lib.info.AlignInfo.UI_HEIGHT - Stage.height) * 0.5 + (Stage.height - _loc3_.y * (lib.info.AlignInfo.scaleRatio / 0.8)) * 0.5;
            target._xscale = lib.info.AlignInfo.scaleRatio / 0.8 * 100;
            target._yscale = lib.info.AlignInfo.scaleRatio / 0.8 * 100;
